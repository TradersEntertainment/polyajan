...
async def run_autonomous_scan_cycle():
    ...
    for m in markets:
        ...
        # 2. Get active parameter tuning from DB
        tuning = await database.get_tuning(symbol, bet_type)
        ...
        # 3. Perform backtests for both UP and DOWN outcomes
        for direction in ["UP", "DOWN"]:
            ...
            # 4. Filter: We look for opportunities where:
            # - Real quant probability of winning is high (e.g. >= 85%) AND expected yield meets the minimum yield
            # - OR there is a significant pricing edge (e.g. real probability is 15%+ higher than Polymarket price)
            is_significant_edge = edge >= 0.12
...
async def agent_scheduler_loop():
    ...
    while True:
        try:
            # 1. Run parameter tuning loop for all assets (once every 4 hours)
            ...
            # 2. Run signal scan loop (every 5 minutes during trading windows)
            # Run scan 6 times per tuning cycle
            for _ in range(48):
